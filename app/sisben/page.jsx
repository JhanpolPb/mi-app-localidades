...
export default function SisbenPage() {
  ...
  const borrar = async (id) => {
    const res = await fetch(`/api/sisben/${id}`, { method: 'DELETE' });
    setConfirmId(null);
    if (res.ok) {
      mostrarMsg('Registro eliminado con éxito ✓', 'success');
      cargar();
    } else {
          ...
          <table className={s.table}>
            ...
            <tbody>
              {datos.length === 0 ? (
                ...
                datos.map((item) => (
                  <tr key={item.ID_SISBEN}>
                    <td>{item.ID_SISBEN}</td>
                    <td>{item.NOMBRE_SISBEN}</td>
                    <td>
                      {confirmId === item.ID_SISBEN ? (
                        <div className={s.actions}>
                          <span style={{ fontSize: '0.85rem', color: '#c92a2a', marginRight: 4 }}>
                            ¿Confirmar?
                          </span>
                          <button className={`${s.btn} ${s.btnDanger}`} onClick={() => borrar(item.ID_SISBEN)}>
                            Sí, borrar
                          </button>
                        ...
                        <button className={`${s.btn} ${s.btnDanger}`} onClick={() => setConfirmId(item.ID_SISBEN)}>
                           Eliminar
                        </button>
                      )}
                    </td>
